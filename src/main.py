...
If a tool is not running make sure you have the right requirements. Check it from requirements.txt file''')
...
class Gui(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.OpenShortcut = QtGui.QShortcut(QKeySequence("Ctrl+N"), self)
        self.OpenShortcut.activated.connect(self.OnNew)
        ...
        self.CursorToolButton.setIconSize(QtCore.QSize(3,3))
        ...
        self.PortraitModeBackgroundBlurToolButton.setToolTip("Portrait Background Blur")
        ...
        self.ColorizerToolButton.setToolTip("Interactive Colourizer")
        ...
        self.AnimeGanToolButton = QToolButton(self)
        self.AnimeGanToolButton.setText("&Anime Style")
        self.AnimeGanToolButton.setToolTip("Anime Style")
        self.setIconPixmapWithColor(self.AnimeGanToolButton, "icons/anime.svg")
        self.AnimeGanToolButton.setCheckable(True)
        self.AnimeGanToolButton.toggled.connect(self.OnAnimeGanToolButton)
        ...
        self.ToolButtons = [
            ...
            self.ColorizerToolButton, self.SuperResolutionToolButton, self.AnimeGanToolButton, 
        ]
        ...
        self.ToolbarDockWidget.setGeometry(QtCore.QRect(5, 15, 90, 550))
    ...
    def createMenuBar(self):
        ...
        helpMenu.addAction(self.helpconAction)
    ...
    def createAction(self):
        ...
        self.newAction.setStatusTip("Add new image for editing")
        self.newAction.setToolTip("Add new image for editing")
        self.newAction.triggered.connect(self.OnNew)

        self.saveAction = QtGui.QAction("&Save", self)
        self.saveAction.setShortcut("Ctrl + S")
        self.saveAction.setStatusTip("Save Image")
        self.saveAction.setToolTip("Save Image")
        self.saveAction.triggered.connect(self.OnSave)

        self.saveasAction = QtGui.QAction("&Save As", self)
        self.saveasAction.setShortcut("Ctrl + Shift + S")
        self.saveasAction.setStatusTip("Save as Newly edited Image")
        self.saveasAction.setToolTip("Save as Newly edited Image")
        ...
        self.helpconAction = QtGui.QAction("&Help Content", self)
        self.helpconAction.triggered.connect(self.onHelpCon)
    ...
    #     self.centralWidget.addAction(self.saveasAction)
    #     self.centralWidget.addAction(self.undoAction)
    #     self.centralWidget.accAction(self.pasteAction)
    def onHelpCon(self):
        self.parent = HelpWindow()
    ...
    def setToolButtonStyleChecked(self, button):
        ...
        button.setStyleSheet('''
            border-width: 2px;
            border-color: rgb(152, 33, 118);
            background-color: rgba(152, 33, 118, 0.60);
            border-style: solid;
        ''')
                    ...
                    if len(filepath) and os.path.isfile(filepath):
                        ...
                        if first.width != second.width or first.height != second.height:
                            ...
                            for button in [resizeFirst, resizeSecond, stackAsIs, cancel]:
                                button.setStyleSheet('''
                                    border: 2px solid;
                                    border-color: rgb(242,198,73);
                                    background-color: rgba(242, 198, 73, 0.80);
                                    height: 30px;
                                    width: 100px;
                                ''')
                            ...
                            msgBox.setStyleSheet('''
                                background-color: rgba(152, 33, 118, 0.2);
                            ''')
                            ret = msgBox.exec()
                            ...
                            if ret == 3:
                                ...
                                first = ImageOps.cover(first, (second.width, second.height))
                            elif ret == 1:
                                # Resize second to match the size of first
                                second = ImageOps.cover(second, (first.width, first.height))
                            elif ret == 2:
                                # Do nothing
                                pass
                        ...
                        if first.width != second.width or first.height != second.height:
                            ...
                            for button in [resizeFirst, resizeSecond, stackAsIs, cancel]:
                                button.setStyleSheet('''
                                    border: 2px solid;
                                    border-color: rgb(242,198,73);
                                    background-color: rgba(242, 198, 73, 0.8);
                                    height: 30px;
                                    width: 100px;
                                ''')
    ...
    @QtCore.pyqtSlot()
    def OnAnimeGanCompleted(self, tool):
        output = tool.output
        if output is not None:
            # Save new pixmap
            updatedPixmap = self.ImageToQPixmap(output)
            self.image_viewer.setImage(updatedPixmap, True, "Anime GAN")

        self.AnimeGanToolButton.setChecked(False)
        del tool
        tool = None

    def OnAnimeGanToolButton(self, checked):
        if checked:
            self.InitTool()
            currentPixmap = self.getCurrentLayerLatestPixmap()
            image = self.QPixmapToImage(currentPixmap)

            from QToolAnimeGAN import QToolAnimeGAN
            widget = QToolAnimeGAN(None, image, self.OnAnimeGanCompleted)
            widget.show()
    ...
    def OnSlidersToolButton(self, checked):
        if checked:
            ...
            self.slidersScroll.setStyleSheet('''
                background-color: rgba(152, 33, 118, 0.35);
            ''')
    ...
    def createLayersDock(self):
        ...
        self.layerListDock = QLayerList("Layer Captured", self)
...
def main():
    ...
    QToolButton:pressed {
        border-width: 2px;
        border-color: rgb(242, 198, 73);
        background-color: rgba(242, 198, 73, 0.65);
        border-style: solid;
    }
    QPushButton {
        border: none;
